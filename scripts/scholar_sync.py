...
# Google Scholar throttles bare requests-library traffic harder than a real
# browser. Sending the header set a browser sends makes the request look less
# like a script. Accept-Encoding deliberately omits "br": requests does not
# decode brotli without an extra package, so advertising it would yield an
# undecodable body that parse_listing() would read as zero rows.
BROWSER_HEADERS = {
